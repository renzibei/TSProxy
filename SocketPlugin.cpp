...
int SocketPlugin::connectSocket(const char *addrStr, int port, int isNonBlocking) {
    ...
    int socketFd = tryConnectSocket(addrStr, port, 0);
    if (isNonBlocking) {
        
    }
    if (socketFd < 0) {
        LogHelper::log(Error, "SocketPlugin Failed to connect");
        return socketFd;
    }
    this->m_sockfd = socketFd;
    return 0;
